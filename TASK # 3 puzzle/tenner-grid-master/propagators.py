...
def is_consistent(csp, var):
    '''Check if all constraints involving the given variable are satisfied'''
    for cons in csp.get_cons_with_var(var):
        if not cons.is_satisfied():
            return False
    return True

def is_complete(csp):
    '''Check if all variables in the CSP are assigned'''
...
def forward_checking(csp, var):
    '''Perform forward checking to make inferences based on the assigned value of the variable'''
    inferences = []

    for cons in csp.get_cons_with_var(var):
        for related_var in cons.get_unassigned_vars():
            for value in related_var.cur_domain():
                if not cons.has_support(related_var, value):
                    related_var.prune_value(value)
                    inferences.append((related_var, value))

            # If the domain of a related variable becomes empty, return None to indicate failure
            if related_var.cur_domain_size() == 0:
                reverse_inferences(csp, inferences)
                return None

    return inferences


def reverse_inferences(csp, inferences):
    '''Reverse the inferences made during forward checking by restoring the pruned values'''
    for var, value in inferences:
        var.restore_value(value)