...
# Initialisation
    ...
    if isinstance(receivers, str):
        with open('receivers.txt', 'w') as fp:
            fp.write(receivers)
...
# Set secretState to True as no secrets are reveiled otherwise False
    ...
    if cm.hasMode('crypted'):
        ...
        for receiver in receivers:
            if receiver in cm.rcvs:
                decrypted_message = cm.rcvs[receiver].decrypt(cm.mesg)
                receiversState[receiver] = cm.rcvs[receiver].getState()
                mesg = decrypted_message.decode('utf-8')
        for sender in senders:
            if sender in cm.snds:
                decrypted_message = cm.snds[sender].decrypt(cm.mesg)
                sendersState[sender] = cm.snds[sender].getState()
                mesg = decrypted_message.decode('utf-8')
    ...
    if cm.hasMode('hashed'):
        ...
        for receiver in receivers:
            currentReceiver = receivers
            if receiver in cm.rcvs:
                verified = cm.rcvs[receiver].verify(cm.mesg)
                receiversState[receiver] = cm.rcvs[receiver].getState()
                hashState = verified
        for sender in senders:
            currentSender = senders
            if sender in cm.snds:
                verified = cm.snds[sender].verify(cm.mesg)
                sendersState[sender] = cm.snds[sender].getState()
                hashState = verified
...
# Convert bytes to str
    ...
    with open('sender.txt', 'r') as fp:
        currentSender = fp.read()
    with open('receivers.txt', 'r') as fp:
        currentReceiver = fp.read()
