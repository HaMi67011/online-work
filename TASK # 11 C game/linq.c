...
void sendMessageToServer(char *ipAddress, int portno, char *mess)
{
    ...
    snprintf(sendbuffer, sizeof(sendbuffer), "C %s %d %.50s", ipAddress, portno, mess);
...
{
    const int MAX_WORD_LENGTH = 50; // Define the maximum word length
    static char word[MAX_WORD_LENGTH] = ""; // Initialize word as an empty string
    static int cptWord = 0; // Initialize cptWord as 0
        ...
        case SDL_KEYDOWN:
            {
                ...
                else if ((car >= 97) && (car <= 122) && (cptWord < MAX_WORD_LENGTH - 1))
                {
                    word[cptWord++] = car;
                    word[cptWord] = '\0';
                }
        ...
        case SDL_MOUSEBUTTONDOWN:
            switch (screenNumber)
            {
                case 0:
                    ...
                    {
                        strncpy(sendBuffer, "C ", sizeof(sendBuffer));
                        strncat(sendBuffer, gClientIpAddress, sizeof(sendBuffer) - strlen(sendBuffer) - 1);
                        snprintf(sendBuffer + strlen(sendBuffer), sizeof(sendBuffer) - strlen(sendBuffer), " %d ", gClientPort);
                        strncat(sendBuffer, gName, sizeof(sendBuffer) - strlen(sendBuffer) - 1);
...
int main(int argc, char** argv)
{
    ...
    if (ret != 0)
    {
        printf("erreur creation thread serveur tcp\n");
        exit(1);
    }
